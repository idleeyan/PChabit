...
public sealed partial class CategoryManagementTab : UserControl
{
    public CategoryManagementViewModel ViewModel { get; }

    public CategoryManagementTab()
    {
        Log.Information("CategoryManagementTab: 构造函数开始");

        ViewModel = App.GetService<CategoryManagementViewModel>();

        InitializeComponent();

        Loaded += CategoryManagementTab_Loaded;

        Log.Information("CategoryManagementTab: 构造函数完成");
    }

    private async void CategoryManagementTab_Loaded(object sender, RoutedEventArgs e)
    {
        try
        {
            await ViewModel.InitializeAsync();
            Log.Information("CategoryManagementTab: 初始化完成");
        }
        catch (Exception ex)
        {
            Log.Error(ex, "CategoryManagementTab: 初始化失败");
        }
    }
