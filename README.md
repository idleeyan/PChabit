...
- **WebDAV 云端同步**: 异地容灾，上传备份到 WebDAV 服务器
...
- **热力图**: 周热力图、月热力图展示使用模式
- **时间线**: 详细的活动记录和时间分布
- **分析页**: 周统计、热力图、智能洞察、应用流向四大标签页
- **应用流向图**: 可视化应用切换关系，支持日期范围和 TopN 筛选
...
### 其他功能
- **系统托盘**: 最小化到系统托盘，后台运行
- **托盘进度环**: 托盘图标实时显示今日目标完成进度
- **深色模式**: 支持浅色/深色/系统默认三主题切换
- **高性能**: 低内存占用，低 CPU 使用率
...
# 使用 MSBuild 编译（推荐）
& "C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\MSBuild\Current\Bin\MSBuild.exe" src/PChabit.App/PChabit.App.csproj /p:Configuration=Release /p:Platform=x64 /restore
```
